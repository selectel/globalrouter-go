...
const (
	...
	appName = "globalrouter-go"

	// defaultGlobalRouterAPIURL represents the default API url for HTTP requests.
	defaultGlobalRouterAPIURL = "https://api.selectel.ru/naas/v1"
...
// ServiceClient stores details that are needed to work with Selectel Global Router API.
type ServiceClient struct {
	...
	// APIUrl represents an GlobalRouter API that will be used in all requests.
	APIUrl string

	// userAgent contains user agent that will be used in all requests.
	userAgent string

	// ClientUserAgent contains a custom user agent prefix that will be added to userAgent.
	ClientUserAgent string
}

// NewClientV1 initializes a new Global Router API client for the V1 API.
func NewClientV1(tokenID string, options ...ServiceClientOption) (*ServiceClient, error) {
	...
	if client.APIUrl == "" {
		client.APIUrl = defaultGlobalRouterAPIURL
	}
	defaultUserAgent := appName + "/" + findModuleVersion()
	client.userAgent = defaultUserAgent
	if client.ClientUserAgent != "" {
		client.userAgent = client.ClientUserAgent + " " + defaultUserAgent
	}
